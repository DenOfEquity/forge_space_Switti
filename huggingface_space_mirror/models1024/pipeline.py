...
from models1024.vqvae import VQVAEHF
from models1024.clip import FrozenCLIPEmbedder
from models1024.switti import SwittiHF, get_crop_condition
from models1024.helpers import sample_with_top_k_top_p_, gumbel_softmax_with_rng


TRAIN_IMAGE_SIZE = (512, 512)

class SwittiPipeline1024:
    ...
    def from_pretrained(cls):
        switti = SwittiHF.from_pretrained("yresearch/Switti-1024", device='cpu').to(torch.bfloat16)
        vae = VQVAEHF.from_pretrained(cls.vae_path, reso=1024).to(torch.bfloat16).to('cpu')
        text_encoder = None
        text_encoder_2 = None
    ...
    def __call__(
        ...
        if self.switti is None:
            self.switti = SwittiHF.from_pretrained("yresearch/Switti-1024", device='cuda').to(torch.bfloat16)
        else:
            self.switti.to('cuda')
        if self.vae is None:
